...
export const EducationSection = ({ userData = { education: [] }, isOwnProfile, onSave }) => {
	...
	const handleAddEducation = () => {
		if (newEducation.school && newEducation.fieldOfStudy && newEducation.startYear) {
			setEducations([...educations, { ...newEducation, _id: Date.now() }]); // Temporary ID for UI purposes
			...
			{isEditing && (
				<div className="mt-4">
					...
					<button
						...
						className="bg-[#084A9E] text-white py-2 px-4 rounded hover:bg-primary-dark transition duration-300"
					>
						Add Education
					</button>
				</div>
			...
			{isOwnProfile && (
				<>
					{isEditing ? (
						<button
							onClick={handleSave}
							className="mt-4 bg-[#084A9E] text-white py-2 px-4 rounded hover:bg-primary-dark transition duration-300"
						>
							Save Changes
						</button>
					) : (
						<button
							...
							className="mt-4 text-[#084A9E] hover:text-primary-dark transition duration-300"
						>
							Edit Education
						</button>
					)}
